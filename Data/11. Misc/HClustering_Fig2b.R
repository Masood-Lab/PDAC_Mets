...
# source(here::here("pdac", "visualization", "eda.R"))
```
...
metadata <- so@meta.data %>%
  select(orig.ident, patient,cc_ischia_10) %>%
  rownames_to_column("row_id")
 
 
# Get the proportions data
rctd_fullfinal <- t(so@assays$rctd_fullfinal@data)
 
# Ensure the data is in the right format
rownames(rctd_fullfinal) <- make.unique(rownames(rctd_fullfinal))
 
# Log transformation of rctd_tier1
log_comps <- log10(rctd_fullfinal)
```
...
cluster_summary_pat <- rctd_fullfinal %>%
  as.data.frame() %>%
  rownames_to_column("row_id") %>%
  left_join(metadata, by = "row_id") %>%  # Join with meta_data using row_id as the key
  pivot_longer(-c(row_id, orig.ident, patient, cc_ischia_10), values_to = "ct_prop", names_to = "cell_type") %>%
  group_by(orig.ident, patient, cc_ischia_10, cell_type) %>%
...
cluster_summary <- cluster_summary_pat %>%
  ungroup() %>%
  group_by(cc_ischia_10, cell_type) %>%
...
cluster_summary_mat <- cluster_summary %>%
  ...
  column_to_rownames("cc_ischia_10") %>%
  as.matrix()
...
run_wilcox_up <- function(prop_data) {
  prop_data_group <- prop_data[["cc_ischia_10"]] %>% unique() %>% set_names()
  map(prop_data_group, function(g) {
    test_data <- prop_data %>%
      mutate(test_group = ifelse(cc_ischia_10 == g, "target", "rest")) %>%
...
mean_ct_prop_plt <- cluster_summary %>%
  left_join(wilcoxon_res, by = c("cc_ischia_10", "cell_type")) %>%
  mutate(cell_type = factor(cell_type, levels = ct_order), cc_ischia_10 = factor(cc_ischia_10, levels = cluster_order)) %>%
  ungroup() %>%
  group_by(cell_type) %>%
  mutate(scaled_pat_median = (patient_median_ct_prop - mean(patient_median_ct_prop)) / sd(patient_median_ct_prop)) %>%
  ungroup() %>%
  ggplot(aes(x = cell_type, y = cc_ischia_10, fill = scaled_pat_median)) +
...
barplts <- cluster_counts %>%
  mutate(cc_ischia_10 = factor(cc_ischia_10, levels = cluster_order)) %>%
  ggplot(aes(y = cc_ischia_10, x = prop_spots)) +
...
plt <- cluster_summary_pat %>%
  ggplot(aes(x = cc_ischia_10, y = median_ct_prop)) +
  geom_boxplot() +
